namespace COMP123_W4_L2
{
    public class Atom
    {
        ...
        {
            string[] atomInfo = objectData.Split(' ');
            string name = atomInfo[0];
            int proton = Convert.ToInt32(atomInfo[1]);
            int neutron = Convert.ToInt32(atomInfo[2]);
            double weight = Convert.ToDouble(atomInfo[3]);
            string symbol = atomInfo[4];
            return new Atom(name, proton, neutron, weight, symbol);
        }
        public static List<Atom> GetAtom()
        {
